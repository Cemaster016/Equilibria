...
pip install -e ".[dev]"
cp .env.example .env
# edit .env and set GROQ_API_KEY or GOOGLE_API_KEY as needed
```
...
```bash
PYTHONPATH=src uvicorn equilibria.api:app --reload --host 0.0.0.0 --port 8000
```
...
Run the demo UI (alternative Streamlit interface):
```bash
