...
async def _pipeline(user_request: str) -> EquilibriaResult:
    ...
    print("DEBUG scored_population length:", len(scored_population_geojson))
    print("DEBUG scored_population preview:", scored_population_geojson[:200])
    ...
    print("DEBUG chosen_sites length:", len(chosen_sites_geojson))
    print("DEBUG chosen_sites preview:", chosen_sites_geojson[:200])
    if error:
        return EquilibriaResult(error=error)

    chosen_sites_geojson = _enrich_sites_with_place_names(chosen_sites_geojson)
    ...
    print("DEBUG map_html length:", len(map_html))
...
def _enrich_sites_with_place_names(chosen_sites_geojson: str) -> str:
    """
    Reverse geocodes each chosen site coordinate to get a real ward/LGA name,
    then adds place_name, lga, state, and full_address properties to each
    GeoJSON feature so the ReportAgent can name them in the brief.
    """
    import time
    ...
    geolocator = Nominatim(user_agent="equilibria-reverse-geocoder")
    data = json.loads(chosen_sites_geojson)
    ...
    for i, feature in enumerate(data.get("features", [])):
        ...
        try:
            ...
            if location and location.raw:
                ...
                feature["properties"]["place_name"] = name
                feature["properties"]["lga"] = lga
                feature["properties"]["state"] = state
                feature["properties"]["full_address"] = location.address
            else:
                feature["properties"]["place_name"] = f"Site {i + 1}"
                feature["properties"]["lga"] = ""
                feature["properties"]["state"] = ""
                feature["properties"]["full_address"] = f"{lat:.4f}, {lon:.4f}"
        except (GeocoderTimedOut, Exception) as exc:
            logger.warning("Reverse geocoding failed for site %d: %s", i + 1, exc)
            feature["properties"]["place_name"] = f"Site {i + 1}"
            feature["properties"]["lga"] = ""
            feature["properties"]["state"] = ""
            feature["properties"]["full_address"] = f"{lat:.4f}, {lon:.4f}"
