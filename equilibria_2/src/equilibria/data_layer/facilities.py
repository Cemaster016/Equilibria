...
def _fetch_from_osm(bbox: BBox) -> gpd.GeoDataFrame:
    ...
    if gdf.empty:
        return gpd.GeoDataFrame(columns=NORMALIZED_COLUMNS, geometry="geometry", crs="EPSG:4326")
...
def _fetch_from_file(path: str, bbox: BBox) -> gpd.GeoDataFrame:
    ...
    name_col = _find_column(gdf, ("name", "facility_name", "facility name", "fname"))
    type_col = _find_column(gdf, ("type", "facility_type", "facility type", "category"))
    gdf["name"] = gdf[name_col].fillna("Unnamed facility") if name_col else "Unnamed facility"
    gdf["facility_type"] = (
