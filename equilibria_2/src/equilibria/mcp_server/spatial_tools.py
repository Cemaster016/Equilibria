...
def equity_score(
    ...
    *,
    ...
    # --- distance score (network distance to nearest facility) -------------
    facility_points: list[Point] = list(facilities_gdf.geometry.values)
    if len(facility_points) > max_facilities_for_distance:
        # Keep the runtime bounded: subsample evenly rather than crash on huge lists.
        facility_points = facility_points[::max(1, len(facility_points) // max_facilities_for_distance)]

    distances = []
    for pt in centroids:
        try:
            d = nearest_facility_distance(road_graph, pt, facility_points)
        except Exception as exc:  # pragma: no cover - defensive, logged not raised
            logger.warning("nearest_facility_distance failed for a cell, using NaN: %s", exc)
            d = np.nan
        distances.append(d)
    distances = np.array(distances, dtype=float)
    # Fill any NaNs (disconnected road graph cells) with the max observed
    # distance so they're treated as "worst case" rather than dropped.
    if np.isnan(distances).any():
        fill_value = np.nanmax(distances) if not np.all(np.isnan(distances)) else 1.0
        distances = np.nan_to_num(distances, nan=fill_value)
