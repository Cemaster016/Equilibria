...
@mcp.tool(
    description=(
        ...
        "GeoJSON strings; the road network must already be cached for the "
        "same area (see fetch_road_network in the data layer). Returns the "
        "population grid as GeoJSON with an added 'equity_score' column."
    )
)
def equity_score(
    pop_geojson: str,
    ...
    pop_gdf = _geojson_to_gdf(pop_geojson)
    facilities_gdf = _geojson_to_gdf(facilities_geojson)
    road_graph = ox.load_graphml(road_graph_path)
...
@mcp.tool(
    description=(
        ...
        "respecting an optional budget. Requires the population grid to "
        "already have an 'equity_score' column (run the equity_score tool "
        "first). Returns the chosen sites as GeoJSON with "
...
def site_allocate(
    ...
    pop_gdf = _geojson_to_gdf(scored_population_geojson)
...
def coverage_map(
    ...
    scored_population_geojson: str,
    existing_facilities_geojson: str,
) -> str:
    chosen_gdf = _geojson_to_gdf(chosen_sites_geojson)
    pop_gdf = _geojson_to_gdf(scored_population_geojson)
    facilities_gdf = _geojson_to_gdf(existing_facilities_geojson)
